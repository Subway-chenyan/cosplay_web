...
function NoVideoAwardCard({ awardRecord, awardInfo }: NoVideoAwardCardProps) {
  ...
  return (
    <div className="max-w-sm relative group/no-video">
      {/* 底部阴影 */}
      <div className="absolute inset-0 bg-black translate-x-1 translate-y-1 z-0"></div>

      <div className="relative z-10 bg-white border-2 border-black overflow-hidden transition-all group-hover/no-video:-translate-y-1">
        {/* 缩略图区域 */}
        <div className="relative aspect-video bg-gray-200 overflow-hidden">
          ...
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
            <div className="text-center">
              <div className="bg-p5-red p-2 border-2 border-black shadow-[4px_4px_0_0_black] mb-2">
                <X className="w-10 h-10 text-white" />
              </div>
              <div className="bg-black text-white px-4 py-1 text-sm font-black">
                <span className="inline-block tracking-tight">暂无视频</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          {awardRecord.drama_name && (
            <h3 className="text-lg font-black text-black mb-2 line-clamp-2 tracking-tight" style={{ textShadow: '1px 1px 0px rgba(0,0,0,0.1)' }}>
              {awardRecord.drama_name}
            </h3>
          )}
          ...
          {awardRecord.group_name && (
            <div className="flex items-center space-x-2 text-xs font-black text-p5-red mb-2">
          ...
          {awardRecord.description && (
            <p className="text-xs font-bold text-gray-500 line-clamp-2 border-l-4 border-black pl-3 py-1">
              {awardRecord.description}
            </p>
          )}
        </div>
      ...
      {/* 奖项标识 */}
      <div className={`absolute -top-2 -right-2 z-20 bg-p5-red text-white p-2 border-2 border-black shadow-[2px_2px_0_0_black]`}>
        {awardInfo.icon}
      </div>

      {/* 年份标识 */}
      {(awardRecord.competition_year_detail?.year || awardRecord.competition_year) && (
        <div className="absolute bottom-2 right-2 z-20 bg-black text-white px-3 py-0.5 font-black text-xs border border-p5-red">
          <span>
